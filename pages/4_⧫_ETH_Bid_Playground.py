...
from models.eth_bid import EthBidModel
...
eth_bid_data = eth_bid_model.run_sim()

circ_supply_timeseries = (
    eth_bid_model.rook_supply.total_supply
    - eth_bid_model.rook_supply.strategic_reserves
    - eth_bid_data["treasury_rook"]
    - eth_bid_data["unclaimed_rook"]
    - eth_bid_data["burned_rook"]
)

mcap_timeseries = circ_supply_timeseries * eth_bid_data["rook_price"]
...
fig.append_trace(go.Scatter(x=eth_bid_data["date"], y=eth_bid_data["rook_price"], name="ROOK Price"), row=1, col=1)

fig.append_trace(
    go.Scatter(x=eth_bid_data["date"], y=eth_bid_data["treasury_eth"], name="Treasury ROOK balance"), row=2, col=1
)

fig.append_trace(go.Scatter(x=eth_bid_data["date"], y=eth_bid_data["daily_volume"], name="Daily Volume"), row=3, col=1)

fig.append_trace(go.Scatter(x=eth_bid_data["date"], y=circ_supply_timeseries, name="Circ Supply"), row=4, col=1)

fig.append_trace(go.Scatter(x=eth_bid_data["date"], y=eth_bid_data["staked_rook"], name="Staked ROOK"), row=1, col=2)

fig.append_trace(
    go.Scatter(x=eth_bid_data["date"], y=eth_bid_data["unclaimed_rook"], name="Unclaimed ROOK"), row=2, col=2
)

fig.append_trace(go.Scatter(x=eth_bid_data["date"], y=eth_bid_data["burned_rook"], name="Burned ROOK"), row=3, col=2)

fig.append_trace(go.Scatter(x=eth_bid_data["date"], y=mcap_timeseries, name="Market Cap"), row=4, col=2)
